...
Если зайти глубже и вновь выполнить ```git status```, то видим:

<img src="./assets/39.png"/> то видим, что эти изменения находятся в самой папке ```example```, но где-то глубже. (Ну и в README.md и картинках (там всегда есть изменения, ведья я тебе это все делаю в процессе написания этого текста))

Ну-ка, а что выведет ```git status```, если спустимся в папку ```./tests```?