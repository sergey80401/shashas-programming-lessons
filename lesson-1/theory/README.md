...
Реализуем нашу функцию. (Да, для себя я выбрал очень простой пример)

<img src="./assets/35.png"/>

Функцию переделали, давай теперь запустим тесты!

Снова ```npm run test``` и готово!

<img src="./assets/36.png"/>

Ну ваще красота!!! На такое даже смотреть приятно!!!

Ну мы же не лошки ебаные, мы хотим, чтобы за нас "сиаечка" все делала.

Я добавлю в файл ```.github/workflows/test.yml``` следующее

<img src="./assets/37.png"/>

А именно джобу для запуска тестов в папке example.

А тперь внимание!!!

Выполню команду ```git status```.

<img src="./assets/38.png"/>

Вывод команды говорит о том, что есть изменения в файле ```.github/workflows/test.yml``` и папке ```example```

Давай сначала закоммитим и запушим измененный ```test.yml```

<img src="./assets/39.png"/>

Видно, что последний пайп упал

<img src="./assets/40.png"/>

Но у нас остались еще приколы, которые мы не запушилиЭ это сами тест и код. Сделаем это!

<img src="./assets/41.png"/>

Коммичу и пушу все остальное. И что вижу на гитхабе?

<img src="./assets/42.png"/>

А там все прошло!!!

И если туда внутрь провалюсь вижу такое.

<img src="./assets/43.png"/>

Добавилась еще одна джоба в пайплайн, которая зелёная!!!

```lesson-1-example-tests``` - как раз-таки мои тесты на функцию ```getSum```

Такс, до этого момента ты даже не должен был писать никакого кода, а теперь, что ты должен будешь сделать, чтобы урок считался пройденным:

Я создал в папке урока папочки ```src``` и ```tests```, в первой уже оставил ```.js``` файлы с задачками. Тебе нужно будет сделать все то же, что и делал я выше. А именно:

1) Инициализировать ```Node``` проект, корнем проекта будет папка ```lesson-1```.
2) Установить ```jest```, поправить вызов скрипта в ```package.json```
3) В папке ```tests``` написать тесты. На каждое задание. По аналогии с ```lesson-0```
4) В ```.github\workflows\test.yml``` Написать новую джобу для пайплайна тестирования. Назвать её ```lesson-1-tests```
5) В ```src``` постараться решить все задачки.

P.S. Задача ```chickiBrickiVDamki``` самая сложная, я только придумал. сам не решал, хахаха. Ну и разумаеется делаем все в отдельной ветки с пул реквестом. А так же почаще делай пуши. Например, сделал сегодня одну задачку, даже если не доконца, закоммить, запуш. Так я смогу отслеживать промежуточный результат работы, и мб где-то подсказать или направить тебя. Ну и задавай вопросы, если будут.
